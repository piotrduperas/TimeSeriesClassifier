...
def run(dataset: str) -> Tuple[float, float]:
    ...
    if path.join("datasets", dataset, f"{dataset}_TEST.arff") not in contents \
            and path.join("datasets", dataset, f"{dataset}_TRAIN.arff") not in contents \
            and path.isdir(path.join("datasets", dataset, dataset)):
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        raise SyntaxError("Usage: python3 download_and_run.py dataset")
    run(sys.argv[1])
